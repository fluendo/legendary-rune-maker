...
namespace Legendary_Rune_Maker.Data.Providers
{
	internal class MetaLolProvider : Provider
	{
		...
		private static async Task<string> GetChampionURL (int championId, Position? pos = null)
			=> $"https://www.metalol.net/champions/lol-build-guide/solo-queue/{(await Riot.GetChampion (championId, "en_US")).Name}/"
		...
		{
			string html = await WebCache.String (await GetChampionURL (championId, position), soft: true);
		...
		{
			var doc = new HtmlDocument ();
			doc.LoadHtml (await WebCache.String (await GetChampionURL (championId, position), soft: true));
